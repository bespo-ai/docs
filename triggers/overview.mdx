...
<Note>
  Older triggers could send a prompt to the agent instead of running a script. Those keep working, but new triggers always run a script — so what happens on every fire is written down in the script itself, rather than being decided fresh each time.
</Note>
