---
title: "Triggers: run your scripts automatically in Script.it"
description: "Triggers let your scripts run on their own — on a schedule, when an HTTP request arrives, or when an event fires in a connected tool like Slack or Gmail."
...
A webhook trigger gives your script a unique URL. When that URL receives an HTTP POST request, the script runs. Any service that can send an HTTP request — Zapier, Make, Stripe, a CI/CD pipeline, a custom script — can fire your automation this way. The request body is available to your script as a payload, so you can pass data along with the trigger. Use webhook triggers when you want an external system to initiate a Script.it automation in response to something it knows about.
...
3. Click **Add Trigger**.
4. Give the trigger a name, choose the trigger type, and fill in the configuration.
