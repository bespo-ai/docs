...
keywords: ["webhook trigger", "HTTP", "POST", "automation", "Script.it", "external service", "curl"]
---

A webhook trigger gives your script a unique URL. When that URL receives an HTTP POST request, Script.it creates a new session and runs the configured action. Any service that can make an HTTP request — a CI/CD pipeline, Zapier, Make, Stripe, or a script of your own — can fire your automation this way. You can also pass data in the request body, which your script can read as a payload.
...
To test your webhook from the command line:

```bash
curl -X POST https://app.script.it/api/webhook/<your-webhook-id> \
  -H "Content-Type: application/json" \
  -d '{"event": "test", "value": 42}'
```

Replace the URL with the webhook URL shown after you create the trigger.
