...
The agent will create the blocks, write the source files, and set up the `flow.script.yaml` — you don't need to touch any files manually.
...
```
The email block is failing with a 401 error. Can you fix the authentication?
```

<Note>
  Script changes are tracked as git snapshots. If the agent makes a change you don't want, you can ask it to revert: "Undo that last change" or "Go back to the version before you added the upload step."
</Note>
...
The agent uses git snapshots taken before each edit, so it can restore an earlier state of any script accurately.
