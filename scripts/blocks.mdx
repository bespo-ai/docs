...
When you open a block run, the output area has three tabs:

- **Timeline** — shows the block's progress while it is running. Use it to see which step is active, whether the block is still working, and where a failure happened if the run stops.
...
<Note>
  Runtime artifacts — files produced during a run — are stored in the session, not the script. This means the same script can be run in many sessions without the outputs conflicting with each other. If a file should persist with the script, store it as a [script asset](/scripts/assets).
</Note>
