...
<CardGroup cols={2}>
  ...
  >
    Analyzes marketing campaign CSV or JSON data, extracts key metrics, generates LLM-powered insights, and posts the summary to Slack.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="Open a template in your workspace">
    Click a template to open it. Script.it copies the template script into your workspace — it creates the `flow.script.yaml`, source files, and any `data_files` — so you have a full, editable copy.
  </Step>
...
<Tip>
  Templates are a great way to learn how scripts are structured. After opening one, browse the blocks and read the source files to see exactly how each step works — then customize from there.
</Tip>
