---
title: "Scripts"
description: "Scripts are reusable workflows built by the AI agent. Each script contains blocks, source files, reusable files, and context settings for your automation."
keywords: ["scripts", "automation", "workflow", "AI agent", "blocks", "SKILL.md"]
---
...
Scripts can also contain **assets**: persistent files such as templates, reference data, configuration, and small memory files that survive across sessions. See [Assets](/scripts/assets) for details.

A script can also carry its own **triggers** — the rules that run it automatically. They appear in the script alongside its blocks, and they're saved with it, so they travel with the script when you share, export, or import it. See [Triggers](/triggers/overview).

## Agent context

Scripts also have context settings that control how much of the script the agent can use in chat. See [Agent context](/scripts/agent-context) for details.
