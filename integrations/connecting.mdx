...
Disconnecting a credit-priced integration stops future renewals but does not refund the current month's charge. If a renewal cannot be paid, Script.it warns the connection owner and eligible billing administrators, then unlinks the connection after a three-day grace period unless billing is restored.
