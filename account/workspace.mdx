...
A shared workspace appears in exactly one of each person's accounts. Members of the owning organization reach it through that organization's account. A person invited from **outside** the organization sees it in their own **Personal** account under **Shared with you** — they collaborate on the same scripts, while their runs use their own credits and their own connections. See [Working in a workspace shared from outside](/account/organizations#working-in-a-workspace-shared-from-outside).
...
Sharing a connection authorizes the account, not just a specific script or location — the people it reaches can use it from any of their sessions and scripts, and the same connection can be shared with multiple workspaces. Who a share reaches depends on who owns the workspace:

```mermaid
flowchart TD
  S["A connection is shared<br/>with a workspace"] --> O{"Who owns the workspace?"}
  O -- "A person's Personal account" --> P["Every invited collaborator<br/>can use it"]
  O -- "An organization" --> C["Only the organization's members<br/>with workspace access can use it.<br/>Collaborators from outside connect<br/>their own tools instead."]
```
